/* ==========================================================================
   СХЕМА ПРОЕКТА WHITECASH ДЛЯ ЯДРА ПАНЕЛИ (panel-core.js).
...
window.ADMIN = {
  ...
  /* Цвета новой плашки по умолчанию — фирменный зелёный Квартал в
     затемнённом варианте: светлый #85bb65 под белыми буквами нечитаем. */
  defaultTone: { bg: '#3F7020', ink: '#FFFFFF' },
  ...
  site: [
    ...
    { key: 'tagline',  label: 'Строка над главным заголовком',
      hint: 'Мелкая строка с точкой на первом экране, например «29 проверенных предложений банков и МФО».' }
  ],
  ...
  load: function () {
    ...
    var cards = OFFERS.map(function (o, i) {
      ...
      return {
        ...
        tone: { bg: (o.tone && o.tone.bg) || '#3F7020', ink: (o.tone && o.tone.ink) || '#FFFFFF' },
  ...
  build: function (m) {
    ...
    var offers = m.CARDS.map(function (c) {
      ...
      var o = {
        ...
        mark: logo ? t(c.mark) : '',
        logo: logo,
        tone: { bg: t(c.tone && c.tone.bg) || '#3F7020', ink: t(c.tone && c.tone.ink) || '#FFFFFF' },
